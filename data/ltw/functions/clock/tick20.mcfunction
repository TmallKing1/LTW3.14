...
execute if score $state mem matches 2 run function ltw:state/2/tick20
...
# 格式化倒计时
scoreboard players operation $countdown_second mem = $countdown mem
scoreboard players operation $countdown_second mem %= #60 mem
scoreboard players operation $countdown_minute mem = $countdown mem
scoreboard players operation $countdown_minute mem /= #60 mem
