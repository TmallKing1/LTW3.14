...
# 若没有待领取的普通物资，则再生成一批
execute unless score $ley_line_disorder mem matches -1 unless entity @e[tag=basic,type=interaction,tag=mats_exist] unless entity @a[tag=has_mats_basic] if entity @a[tag=mini_running] run function mini:river/game/new_mats
...
# 成功过河的玩家
execute if score $ley_line_disorder mem matches -1 as @a[x=1001,y=18,z=16001,dx=5,dy=5,dz=29,tag=mats_area] at @s run function mini:river/game/cross_success
execute if score $ley_line_disorder mem matches -1 as @a[x=1025,y=18,z=16001,dx=5,dy=5,dz=29,tag=!mats_area] at @s run function mini:river/game/cross_success

# 给予进度
execute if score $timer_max mem matches ..10 unless score #gamemode mem matches 2 run advancement grant @a[tag=mini_running,scores={tran_mats=1..}] only ltw:parkour/river2