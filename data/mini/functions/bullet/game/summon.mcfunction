...
execute if score $random mem matches 1..70 unless score $ley_line_disorder mem matches -1 run summon zombie 1010.0 69 18010.0 {Tags:["new_entity","bullet_entity"]}
execute if score $random mem matches 1..70 if score $ley_line_disorder mem matches -1 run summon skeleton 1010.0 69 18010.0 {Tags:["new_entity","bullet_entity"]}
...
attribute @e[tag=new_entity,limit=1,type=!skeleton] generic.follow_range base set 0
...
item replace entity @e[tag=new_entity,limit=1,type=skeleton] weapon.mainhand with bow
...
# 加入队伍
team join mobs @e[tag=new_entity]

# 村民的额外处理
scoreboard players operation @e[type=villager,tag=new_entity] villager_id = $ villager_id
...
scoreboard players operation @e[type=silverfish,tag=new_entity] villager_id = $ villager_id
effect give @e[type=silverfish,tag=new_entity] invisibility infinite 0 true
execute as @e[type=villager,tag=new_entity] run scoreboard players add $ villager_id 1
