...
# 检测不死图腾
execute if score @s[tag=!have_extra] mini_heart matches 1 if entity @s[nbt={SelectedItem: {id: "minecraft:totem_of_undying"}}] run tag @s add have_totem
execute if score @s[tag=!have_extra] mini_heart matches 1 if entity @s[nbt={Inventory: [{Slot: -106b, id: "minecraft:totem_of_undying"}]}] run tag @s add have_totem
