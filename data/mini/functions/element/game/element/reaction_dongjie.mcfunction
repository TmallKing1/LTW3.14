...
scoreboard players set @e[tag=found_marker,limit=1] countdown 4
data modify entity @e[tag=found_marker,limit=1] data.element_attach set value 0
tp @e[tag=found_marker,limit=1] @s
summon text_display ~ ~1 ~ {billboard:"center",text:'{"text": "冻结","color": "#97FFFF"}',Tags:["text_display"], brightness: {block: 15, sky: 15}, background: 0}
