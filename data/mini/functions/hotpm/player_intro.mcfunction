...
execute if score $ley_line_disorder mem matches -1 run tellraw @s ["",{"text":"\n >> 炸弹狂魔 >>  ","color":"dark_red","bold":true},{"text": "作者：Hex-Dragon","color": "aqua"},"\n\n 炸弹分为有害和无害两种类型，\n 只要 ",{"text":"传递炸弹给他人","color":"gold"}," 就有机会改变炸弹的类型！\n 努力存活下来吧！\n\n",{"text": " <只能击退玩家> ","color":"yellow"},"\n "]