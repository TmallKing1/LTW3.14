# 荆棘检测：仅在 玩家副手物品附有荆棘附魔 且 玩家当前未持有炸弹 时触发荆棘惩罚
tag @s remove thorns_check_offhand
execute if entity @s[tag=pm_holding] run return fail
execute unless data entity @s Inventory[{Slot: -106b}].tag.Enchantments[{id:"minecraft:thorns"}] run return fail

# 物品最大耐久度：钻石护腿为 495，皮革外套为 80
execute if data entity @s Inventory[{Slot: -106b, id:"minecraft:diamond_leggings"}] run scoreboard players set $max_damage mem 495
execute if data entity @s Inventory[{Slot: -106b, id:"minecraft:leather_chestplate"}] run scoreboard players set $max_damage mem 80

# 获取物品当前耐久度
execute store result score $current_damage mem run data get entity @s Inventory[{Slot: -106b}].tag.Damage
...
execute unless score $current_damage mem >= $max_damage mem run item replace entity @s weapon.offhand from block 0 0 1 container.0
execute if score $current_damage mem >= $max_damage mem run item replace entity @s weapon.offhand with air
