...
# 180s：教学
execute if score $countdown mem matches 180 run tellraw @s[team=playing,scores={stat_level=..10,emerald=0}] [{"text":"\n>> ","color":"light_purple","bold": true},{"text":"提示：手持最后一格的交易契约召唤村民，与村民交易，尽量获取绿宝石！","bold":false},"\n "]
execute if score $countdown mem matches 180 run tellraw @s[team=playing,scores={stat_level=..10,emerald=1..}] [{"text":"\n>> ","color":"light_purple","bold": true},{"text":"提示：地牢的箱子中有积分与不少资源，寻找地牢也是不错的选择！","bold":false},"\n "]
...
execute as @a[team=playing] store result score @s temp2 run clear @s nether_star{trade_score: 1b}
...
execute as @e[tag=TradePlayer,tag=!adv4_checked] unless entity @s[nbt={Offers: {Recipes: [{sell: {id: "minecraft:emerald"}, uses: 0}]}}] unless entity @s[nbt={Offers: {Recipes: [{sell: {id: "minecraft:nether_star"}, uses: 0}]}}] run tag @s add adv4_achieved
execute as @a[team=playing] if score @s player_id = @e[tag=TradePlayer,tag=adv4_achieved,tag=!adv4_checked,limit=1] player_id run advancement grant @s only ltw:vs/emerald4
