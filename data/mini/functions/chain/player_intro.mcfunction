...
execute if score $ley_line_disorder mem matches 3 run tellraw @s ["",{"text": " ▶ 幻境干扰: ","color": "#7B68EE"},{"text": "只有即将受到爆炸影响（Boss 条为红色）的玩家才拥有增益效果。\n "}]

execute if score $ley_line_disorder mem matches -1 run tellraw @s ["",{"text":"\n >> 连锁爆破 >>  ","color":"dark_red","bold":true},{"text": "地图：Hex-Dragon  作者：小猪之最Thepig","color": "aqua"},"\n\n TNT 全部为隐形状态，\n 你只能通过 ",{"text": "与最近 TNT 的连接线","color": "gold"}," 判断 TNT 的位置！\n\n",{"text": " <只能击退玩家> ","color":"yellow"},"\n "]