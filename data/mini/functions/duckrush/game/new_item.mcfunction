...
# 显示信息
execute if score $double_reward mem matches 0 run tellraw @a ["",{"text": ">> ","color":"red","bold": true},{"text":"红色鸭子掉落了奖励物资：","color":"red"},{"selector": "@e[tag=bonus_item,limit=1,sort=nearest]"}]
execute if score $double_reward mem matches 1 run tellraw @a ["",{"text": ">> ","color":"red","bold": true},{"text":"红色鸭子掉落了奖励物资：","color":"red"},{"selector": "@e[tag=bonus_item,limit=2,sort=nearest]"}]