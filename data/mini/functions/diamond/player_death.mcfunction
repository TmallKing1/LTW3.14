...
execute store result storage mini:final Return.Count int 1 run scoreboard players operation #diamond_count temp /= #2 mem
execute if score #diamond_count temp matches 1.. run function mini:diamond/game/give_diamond with storage mini:final Return
# 删除marker
kill @e[type=armor_stand,tag=dead_marker]