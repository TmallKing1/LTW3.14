...
execute if score #bonus_add_score mem matches -1 run scoreboard players set @s[scores={total_score=1..}] total_score 0
execute if score #bonus_add_score mem matches -1 run scoreboard players set @s[scores={total_score=1..}] total_score_disp 0
...
# 开方
execute if score #bonus_add_score mem matches -3 if score @s total_score matches ..-1 run tellraw @a [{"text":"","color":"red"},{"text": ">> ","color":"red","bold": true},{"selector": "@s","color":"white"}, " 的积分无法被开方……"]
execute if score #bonus_add_score mem matches -3 unless score @s total_score matches ..-1 run scoreboard players operation $square mem = @s total_score
execute if score #bonus_add_score mem matches -3 unless score @s total_score matches ..-1 run function lib:sqrt
execute if score #bonus_add_score mem matches -3 unless score @s total_score matches ..-1 run scoreboard players operation @s total_score = $square_root mem
execute if score #bonus_add_score mem matches -3 unless score @s total_score matches ..-1 run scoreboard players operation @s total_score_disp = $square_root mem
execute if score #bonus_add_score mem matches -3 unless score @s total_score matches ..-1 run tellraw @a [{"text":"","color":"red"},{"text": ">> ","color":"red","bold": true},{"selector": "@s","color":"white"}, " 的积分惨遭开方，当前共有 ",{"score": {"name": "@s","objective": "total_score"}}," 积分"]
