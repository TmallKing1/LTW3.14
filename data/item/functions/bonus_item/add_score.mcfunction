...
execute un#as player
execute store result score #bonus_add_score mem run data get storage item:bonus item.tag.bonus_add_score

# 正常的加分
execute if score #bonus_add_score mem matches 1..10 run scoreboard players operation @s total_score += #bonus_add_score mem
execute if score #bonus_add_score mem matches 1..10 run scoreboard players operation @s total_score_disp += #bonus_add_score mem
execute if score #bonus_add_score mem matches 1..10 run tellraw @a ["",{"text": ">> ","color":"aqua","bold": true},{"translate": "ltw.game.hint.bonus.acquired.score", "with":[{"selector": "@s","color":"aqua"},[{"translate":"ltw.item.game.score","with":[{"score": {"name": "#bonus_add_score","objective": "mem"}}],"fallback": "%s 积分","color": "aqua"}],{"score": {"name":"@s","objective": "total_score"},"color":"aqua"}],"fallback": "%1$s 获得了 %2$s，当前共有 %3$s 积分"}]

# 没有加分
execute if score #bonus_add_score mem matches 0 run tellraw @a ["",{"text": ">> ","color":"red","bold": true},{"translate":"ltw.game.hint.bonus.bad_luck","fallback": "%s 太非了，没能获得积分……","color": "red","with":[{"selector": "@s","color":"red"}]}]

# 清零
execute if score #bonus_add_score mem matches -1 if score @s total_score matches 0 run tellraw @a [{"text":"","color":"red"},{"text": ">> ","color":"red","bold": true},{"translate":"ltw.game.hint.bonus.clear_zero","fallback": "%s 没能获得积分，因为他没有积分可供清零……","with":[{"selector": "@s","color":"white"}]}]
execute if score #bonus_add_score mem matches -1 if score @s total_score matches 1.. run tellraw @a [{"text":"","color":"red"},{"text": ">> ","color":"red","bold": true},{"translate":"ltw.game.hint.bonus.clear_score","fallback": "%1$s 的 %2$s 积分惨遭清零……","with":[{"selector": "@s","color":"white"},{"score": {"name": "@s","objective": "total_score"}}]}]
execute if score #bonus_add_score mem matches -1 run scoreboard players set @s total_score 0
execute if score #bonus_add_score mem matches -1 run scoreboard players set @s total_score_disp 0
...
execute if score #bonus_add_score mem matches -3 run scoreboard players operation @s total_score = $square_root mem
execute if score #bonus_add_score mem matches -3 run scoreboard players operation @s total_score_disp = $square_root mem
execute if score #bonus_add_score mem matches -3 run tellraw @a [{"text":"","color":"red"},{"text": ">> ","color":"red","bold": true},{"translate":"ltw.game.hint.bonus.square","fallback": "%1$s 的积分惨遭开方，当前共有 %2$s 积分","with":[{"selector": "@s","color":"white"},{"score": {"name": "@s","objective": "total_score"}}]}]
