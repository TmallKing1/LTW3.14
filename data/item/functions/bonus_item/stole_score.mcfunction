...
# 显示提示
execute if entity @s[tag=highest] run tellraw @a ["",{"text": ">> ","color":"aqua","bold": true},{"selector": "@s","color":"aqua"},{"text": " 保护住了自己的分数！"}]
execute if entity @s[tag=!highest] run tellraw @a ["",{"text": ">> ","color":"green","bold": true},{"selector": "@s","color":"green"}," 偷到了 ",{"score": {"name": "#bonus_stole_score","objective": "mem"},"color": "green"},{"text": " 积分"},"，当前共有 ",{"score": {"name": "@s","objective": "total_score"}}," 积分"]
execute if entity @s[tag=!highest] run tellraw @a ["",{"text": ">> ","color":"red","bold": true},{"selector": "@p[tag=highest]","color":"red"}," 被偷走了 ",{"score": {"name": "#bonus_stole_score","objective": "mem"},"color": "red"},{"text": " 积分"},"，当前还剩 ",{"score": {"name": "@p[tag=highest]","objective": "total_score"}}," 积分"]